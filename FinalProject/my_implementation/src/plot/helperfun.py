...
#     score_type = 'P@TopK'
#     finite_indicator = np.isfinite(df['Score'])
#     print(f"Finite Indicator (first 10 rows):\n{finite_indicator.head(10)}")  # Debug

#     for gsc in gsc_lst:
#         gsc_indicator = df['Geneset Collection'] == gsc
#         print(f"Processing Geneset Collection: {gsc}")
#         for network in network_lst:
#             gsc_net_indicator = gsc_indicator & (df['Network'] == network)
#             print(f"Processing Network: {network}")
#             for valsplit in valsplit_dict:
#                 if (gsc == 'GOBPtmp') & (valsplit == '5FCV'):
#                     print("Skipping GOBPtmp with 5FCV.")
#                     continue
#                 gsc_net_val_st_indicator = (
#                     gsc_net_indicator &
#                     (df['Validation Split'] == valsplit) &
#                     (df['Score Type'] == score_type)
#                 )
#                 filtered_df = df[gsc_net_val_st_indicator & finite_indicator]
#                 print(f"Filtered DataFrame (first 10 rows):\n{filtered_df.head(10)}")

#                 if filtered_df.empty:
#                     print(f"No rows match for gsc={gsc}, network={network}, valsplit={valsplit}.")
#                 else:
#                     min_val = filtered_df['Score'].values.min()
#                     print(f"Minimum value for gsc={gsc}, network={network}, valsplit={valsplit}: {min_val}")
#                     df.loc[gsc_net_val_st_indicator & ~finite_indicator, 'Score'] = min_val
